...
const PersonalComponent = (): JSX.Element => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3300/users`, {
      ...
      .then((data) => {
        if (data) {
          setb(
            data.personal_todos.filter((item: any) => {
  ...
  let list = b.map((item: Item, index) => {
    return (
      <div
        className="w-full rounded-md h-150 p-4 flex flex-col items-center shadow-md self-center bg-white my-2"
        key={index}
        onClick={(e) => {
          let target = e.target as HTMLDivElement;
          if (target.id !== "span") {
        ...
        {load.modal === 1 ? (
          ...
          >
            <div className="flex flex-row items-center justify-between w-full border-b border-gray-200 p-4">
              <p className="font-pop text-lg text-left self-start">Personal</p>
              <i
